...
router.get("/:id/students", async (req, res, next) => {
  ...
  let foudnStudent;
  try {
    foudnStudent = await Student.findByPk(id);
  } catch (err) {
    next(err);
  }

  try {
    const campusesOfStudents = await foudnStudent.getStudents();
    res.status(200).json(campusesOfStudents);
  } catch (err) {
    next(err);
  }
...
router.post("/", async (req, res, next) => {
  ...
  try {
    ...
    const newStudent = await Campus.create(studentObj);
